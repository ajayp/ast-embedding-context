...
This POC provides an idea for how semantic search and structural code analysis can be combined to build more intelligent error analysis and debugging tools.

This is a Python script demonstrating a POC retrieving relevant code context and similar historical execution steps (stack frames) based on a given error stack trace.
...
## Why Use ASTs (Abstract Syntax Trees)?

While sentence embeddings are excellent at capturing the *semantic meaning* or *what* the code intends to do, they don't inherently understand the code's *structure* or *control flow*. An Abstract Syntax Tree (AST) is a tree-like programmatic structure representing the syntactic shape of code.  

Parsing code into an AST lets us extract structural features — based on their arrangement in the AST tree structure — like:
...
* **In-Memory Vector Store:** Stores code and stack frame data along with their embeddings and AST features in a simple in-memory list.
* **Context Retrieval:** Given a new error stack trace, it embeds its frames and searches the In-Memory Vector Store for similar code snippets and historical stack frames based on cosine similarity. This leverages the **semantic** information.
...
4.  **Storage:** The original content, metadata, embeddings, and AST features (for code) are stored in a simple in-memory list acting as a In-Memory Vector Store.
5.  **Retrieval:** When a new error stack trace occurs:
    * Each frame of the new stack trace is embedded.
    * These frame embeddings are used to query the In-Memory Vector Store.
...
* The use of AST features in retrieval is currently just for highlighting in the output. In a real system, AST features could be used for re-ranking search results, filtering, or providing structured input to a LLM for deeper analysis of the error in the context of the code's structure.
